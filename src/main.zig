...
pub fn main() !void {
    ...
    for ([_]usize{ 10, 100, 1000, 10_000, 100_000, 1000_000 }) |N| {
        const N_inv: f64 = 1.0 / @as(f64, @floatFromInt(N));
        ...
        const t_a_fill = @as(f64, @floatFromInt(timer.lap())) * 1e-0 * N_inv;

        for (0..N) |i| {
            const ix = try h.insert(gpa, @intCast(i));
            if (rand.boolean()) try h_toerase.append(gpa, ix);
        }
        const t_h_fill = @as(f64, @floatFromInt(timer.lap())) * 1e-0 * N_inv;

        for (a.items) |x| acc +%= 7 * x;
        const t_a_iterfull = @as(f64, @floatFromInt(timer.lap())) * 1e-0 * N_inv;

        var it = h.iterator();
        while (it.next()) |kv| acc +%= 3 * kv.value_ptr.*;
        const t_h_iterfull = @as(f64, @floatFromInt(timer.lap())) * 1e-0 * N_inv;
        ...
        const t_a_erase = @as(f64, @floatFromInt(timer.lap())) * 1e-0 * N_inv;

        for (h_toerase.items) |ix| _ = h.erase(gpa, ix);
        const t_h_erase = @as(f64, @floatFromInt(timer.lap())) * 1e-0 * N_inv;

        for (a.items) |x| acc +%= 7 * x;
        const t_a_iterhalf = @as(f64, @floatFromInt(timer.lap())) * 1e-0 * N_inv;

        it = h.iterator();
        while (it.next()) |kv| acc +%= 3 * kv.value_ptr.*;
        const t_h_iterhalf = @as(f64, @floatFromInt(timer.lap())) * 1e-0 * N_inv;

        for (0..h_toerase.items.len) |i| {
            try a.append(gpa, i);
        }
        const t_a_refill = @as(f64, @floatFromInt(timer.lap())) * 1e-0 * N_inv;

        for (0..h_toerase.items.len) |i| {
            _ = try h.insert(gpa, i);
        }
        const t_h_refill = @as(f64, @floatFromInt(timer.lap())) * 1e-0 * N_inv;
